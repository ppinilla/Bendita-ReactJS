...
const ItemListContainer = ({ greeting }) => {
  ...
  useEffect(() => {
    const getProds = new Promise((resolve) => {
        setTimeout(() => {
          resolve(products)
        }, 1000);
    });
    if(id) {
      getProds.then(p => setProds(p.filter(product => product.category === id)))
    }else {
        getProds.then(p => setProds(p))
      }
}, [id]);
